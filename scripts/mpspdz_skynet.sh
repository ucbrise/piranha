...
kill_n_build () {
    scp -i $SSH_KEY $FILE $USERNAME@$1:/home/azureuser/MP-SPDZ/Programs/Source/ >> /dev/null
    local CMD="sudo pkill -f semi2k-party.x; sudo pkill -f replicated-ring-party.x; sudo pkill -f rep4-ring-party.x; cd MP-SPDZ; rm Programs/Bytecode/benchmark_piranha*; rm Programs/Schedules/benchmark_piranha*; ./compile.py -R 64 benchmark_piranha"
...
if [[ $RUN_TYPE = 2PC ]]; then
    ...
    run_experiment $IP2 "./semi2k-party.x 1 benchmark_piranha -ip ip_lan -N 2 > ./time.txt" &> /dev/null &
    run_experiment $IP1 "./semi2k-party.x 0 benchmark_piranha -ip ip_lan -N 2 > ./time.txt; less time.txt"
